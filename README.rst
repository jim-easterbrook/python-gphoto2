...
You can show the documentation of a function in several ways:

- from the Python interpreter, using their ``__doc__`` attribute (ie. ``print(gphoto2.gp_camera_folder_list_files.__doc__)``);
- from the CLI, using the ``pydoc`` command (or ``python -m pydoc`` if you installed gphoto2 with pip inside a virtual environement)::
